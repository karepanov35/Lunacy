...
use pocketmine\command\utils\CommandParameterProvider;
...
class GiveCommand extends VanillaCommand implements CommandParameterProvider{
	...
	public function getCommandOverloads(int $protocolId) : array{
		...
		foreach(\pocketmine\inventory\CreativeInventory::getInstance()->getAll() as $item){
			$aliases = $parser->lookupAliases($item);
			if(count($aliases) > 0){
				$items[] = $aliases[0];
			}
		}
			...
			new \pocketmine\network\mcpe\protocol\types\command\CommandOverload(
				...
				parameters: [
					\pocketmine\network\mcpe\protocol\types\command\CommandParameter::standard(
						"player",
						\pocketmine\network\mcpe\protocol\AvailableCommandsPacket::convertArg($protocolId, \pocketmine\network\mcpe\protocol\AvailableCommandsPacket::ARG_TYPE_TARGET),
						0,
						false
					),
					\pocketmine\network\mcpe\protocol\types\command\CommandParameter::enum(
						"item",
						new \pocketmine\network\mcpe\protocol\types\command\CommandHardEnum("Item", $items),
						0,
						false
					),
					\pocketmine\network\mcpe\protocol\types\command\CommandParameter::standard(
						"amount",
						\pocketmine\network\mcpe\protocol\AvailableCommandsPacket::convertArg($protocolId, \pocketmine\network\mcpe\protocol\AvailableCommandsPacket::ARG_TYPE_INT),
						0,
						true
					),
					\pocketmine\network\mcpe\protocol\types\command\CommandParameter::standard(
						"data",
						\pocketmine\network\mcpe\protocol\AvailableCommandsPacket::convertArg($protocolId, \pocketmine\network\mcpe\protocol\AvailableCommandsPacket::ARG_TYPE_JSON),
						0,
						true
					)
				]
