...
class NetworkSession{
	 ...
	 */
	private function getDimensionId() : int{
		if($this->player === null){
	...
	private function onSessionStartSuccess() : void{
		...
		$this->setHandler(new LoginPacketHandler(
			...
			function(PlayerInfo $info) : void{
				$this->info = $info;
				// Убрано сообщение "Игрок: {name}" для Lunacy
	...
	private function setAuthenticationStatus(bool $authenticated, bool $authRequired, Translatable|string|null $error, ?string $clientPubKey) : void{
		...
		if(EncryptionContext::$ENABLED && $clientPubKey !== null){
	...
	public function syncAvailableCommands() : void{
		...
		foreach($this->server->getCommandMap()->getCommands() as $command){
			...
			$overloads = [];
			if($command instanceof \pocketmine\command\utils\CommandParameterProvider){
				$overloads = $command->getCommandOverloads($this->getProtocolId());
			}else{
				$overloads = [
