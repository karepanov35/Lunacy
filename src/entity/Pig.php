...
class Pig extends Living implements Ageable{
	...
	/** Ванильно: свинья идёт за игроком с морковью/картофелем/свёклой в радиусе 8 блоков */
	private function checkTempt() : bool{
		...
		foreach($this->getWorld()->getPlayers() as $player){
			...
			if($distance < $nearestDistance && $this->isHoldingEdible($player)){
	...
	private function isHoldingEdible(Player $player) : bool{
		$item = $player->getInventory()->getItemInHand();
		$id = $item->getTypeId();
		return $id === VanillaItems::CARROT()->getTypeId()
			|| $id === VanillaItems::POTATO()->getTypeId()
			|| $id === VanillaItems::BEETROOT()->getTypeId();
	}
	...
	/** Как у коровы: проверка блока впереди (nextX, nextZ — точка впереди по направлению движения) */
	...
	/** Прыжок на блок: выше чем у коровы (0.58), плюс горизонтальный импульс вперёд, чтобы долететь и не трястись */
	...
	public function onInteract(Player $player, Vector3 $clickPos) : bool{
		...
		$isEdible = $itemId === VanillaItems::CARROT()->getTypeId()
			|| $itemId === VanillaItems::POTATO()->getTypeId()
			|| $itemId === VanillaItems::BEETROOT()->getTypeId();
