...
class BrownMushroom implements Populator{
	...
	public function populate(ChunkManager $world, int $chunkX, int $chunkZ, Random $random) : void{
		...
		for($i = 0; $i < $amount; ++$i){
			...
			if($y !== -1 && $this->canMushroomStay($world, $x, $y, $z)){
				$world->setBlockAt($x, $y, $z, VanillaBlocks::BROWN_MUSHROOM());
			}
		}
	}

	private function canMushroomStay(ChunkManager $world, int $x, int $y, int $z) : bool{
		$block = $world->getBlockAt($x, $y, $z);
		$below = $world->getBlockAt($x, $y - 1, $z);
		
		return $block->canBeReplaced() && $below->hasSameTypeId(VanillaBlocks::GRASS());
	}
