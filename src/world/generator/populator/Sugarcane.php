...
class Sugarcane implements Populator{
	...
	public function populate(ChunkManager $world, int $chunkX, int $chunkZ, Random $random) : void{
		...
		for($i = 0; $i < $amount; ++$i){
			...
			if($y !== -1){
				...
				for(; $y < 127 && $y < $yMax; $y++){
					if($this->canSugarcaneStay($world, $x, $y, $z)){
						$world->setBlockAt($x, $y, $z, VanillaBlocks::SUGARCANE());
					}
				}
			}
		}
	...
	private function canSugarcaneStay(ChunkManager $world, int $x, int $y, int $z) : bool{
		...
		foreach($adjacentBlocks as $adjacent){
			if($adjacent->hasSameTypeId(VanillaBlocks::WATER())){
				$water = true;
				break;
			}
		}
		...
		$belowIsGrass = $below->hasSameTypeId(VanillaBlocks::GRASS());
		$belowIsSugarcane = $below->hasSameTypeId(VanillaBlocks::SUGARCANE());
		
		return $block->canBeReplaced() && ((($belowIsSand || $belowIsGrass) && $water) || $belowIsSugarcane);
	}
