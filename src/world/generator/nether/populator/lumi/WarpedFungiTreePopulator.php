...
/** Порт {@code WarpedFungiTreePopulator} (Lumi). */
final class WarpedFungiTreePopulator implements VanillaPopulator{
	...
	public function populate(ChunkManager $world, Random $random, int $chunk_x, int $chunk_z, Chunk $chunk) : void{
		...
		for($i = 0; $i < $amount; ++$i){
			...
			foreach($this->getHighestWorkableBlocks($world, $x, $z) as $y){
				...
				$tree = TreeFactory::get($random, TreeType::WARPED);
	...
	private function getHighestWorkableBlocks(ChunkManager $world, int $x, int $z) : array{
		...
		for($y = $world->getMaxY() - 1; $y > 0; --$y){
			...
			if($b === BlockTypeIds::WARPED_NYLIUM && $world->getBlockAt($x, $y + 1, $z)->getTypeId() === BlockTypeIds::AIR){
				$ys[] = $y + 1;
			}
		}
		return $ys;
