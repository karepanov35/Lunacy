...
class LevelDB extends BaseWorldProvider implements WritableWorldProvider{
	...
	public function __construct(string $path, \Logger $logger){
		...
		try{
			...
			if(str_contains($msg, 'IO error') || str_contains($msg, 'dbtmp')){
				$dbPath = Path::join($path, "db");
				$msg .= " LevelDB cannot open the world database. Make sure no other server is using this world, then try: (1) Stop all server processes, (2) Delete the file " . Path::join($dbPath, "LOCK") . " if it exists, (3) Delete any .dbtmp files in the db folder. If the problem persists, backup and remove the world folder to create a fresh world.";
			}
			throw new CorruptedWorldException($msg, 0, $e);
		}
	}
