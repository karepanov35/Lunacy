...
class Witch extends Living{
	...
	/** Тиков до броска зелья (2 сек = 40). Сначала ведьма берёт зелье в руку, через 2 сек бросает. */
	...
	private function combatAI() : void{
		...
		// Смотреть на уровень груди цели, не вверх на голову — ограничиваем угол по вертикали
		...
		if($this->throwCooldown <= 0 && $this->canSee($this->target) && $dist >= 4.5 && $dist <= 14.0){
			if($this->prepareThrowTicks > 0){
				// ещё готовимся — таймер тикает в entityBaseTick, не сбрасываем
			}elseif(!$this->heldItem->isNull()){
				// зелье в руке и 2 сек прошло — бросаем
				$this->throwPotion();
				...
				// берём зелье в руку, через 2 сек бросим
