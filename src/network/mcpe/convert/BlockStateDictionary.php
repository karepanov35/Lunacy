...
final class BlockStateDictionary{
	...
	public function __construct(
		...
		// Seagrass added in Update Aquatic (1.13); fallback if missing from protocol palette (e.g. older bedrock-data)
		if(!isset($this->stateDataToStateIdLookup[BlockTypeNames::SEAGRASS])){
			$seaPickle = $this->stateDataToStateIdLookup[BlockTypeNames::SEA_PICKLE] ?? null;
			if($seaPickle !== null){
				$this->stateDataToStateIdLookup[BlockTypeNames::SEAGRASS] = is_int($seaPickle)
					? $seaPickle
					: $seaPickle[array_key_first($seaPickle)];
			}
		}
	}
