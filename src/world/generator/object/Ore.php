...
class Ore{
	...
	public function canPlaceObject(ChunkManager $world, int $x, int $y, int $z) : bool{
		return $world->getBlockAt($x, $y, $z)->getTypeId() === $this->type->target_type;
	}

	public function placeObject(ChunkManager $world, int $x, int $y, int $z) : void{
		$clusterSize = $this->type->amount;
		...
		$tried = [];
		$replaceableStateIds = [];
		$materialStateId = $this->type->type->getStateId();
					...
					if($yy > 0 && ($sizeX + $sizeY) < 1){
						for($zz = $startZ; $zz <= $endZ; ++$zz){
							...
							if(($sizeX + $sizeY + $sizeZ) < 1){
								...
								$replaceable = $replaceableStateIds[$stateId] ??= $world->getBlockAt($xx, $yy, $zz)->getTypeId() === $this->type->target_type;
								if($replaceable){
